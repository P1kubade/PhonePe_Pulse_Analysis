...
if view_mode == "Live Pulse (Home)":
    ...
    with tab1:
        ...
        if not cat_data.empty:
            st.plotly_chart(px.pie(cat_data, values='Total_Amount', names='Transaction_Type', hole=0.4), use_container_width=True)
    ...
    with tab2:
        ...
        with c_u_brand:
            ...
            if not brand_data.empty:
                st.plotly_chart(px.pie(brand_data, values='Total_Users', names='Brand', hole=0.4), use_container_width=True)
...
# ==========================================
    ...
    if scenario == "1. Transaction Dynamics":
        ...
        st.plotly_chart(px.line(df.trans_trend_national(), x="Year", y="Total_Amount", title="Is the digital economy growing?"), use_container_width=True)
        
        c1, c2 = st.columns(2)
        with c1: 
            st.subheader("2. Category Split")
            st.plotly_chart(px.pie(df.trans_category_split(year, quarter), values='Total_Amount', names='Transaction_Type', title="Where are people spending?"), use_container_width=True)
        with c2:
            st.subheader("3. State Heatmap")
            st.plotly_chart(plot_india_map(df.trans_state_heatmap(year, quarter), 'Total_Amount', 'State', "Geographic Distribution"), use_container_width=True)
            
        st.subheader("4. Top 10 Districts (Volume)")
        st.plotly_chart(px.bar(df.trans_district_top10(year, quarter), x='District', y='Total_Count', color='Total_Count'), use_container_width=True)
        
        st.subheader("5. Average Transaction Value (ATV)")
        st.plotly_chart(px.area(df.trans_avg_value_trend(), x='Year', y='Avg_Value', title="Are transaction sizes increasing?"), use_container_width=True)
    ...
    # --- SCENARIO 2: DEVICE & USER ENGAGEMENT ---
        ...
        with c1:
            ...
            st.plotly_chart(px.pie(df.user_brand_share(year), values='Total_Users', names='Brand', hole=0.4), use_container_width=True)
        with c2:
            st.subheader("2. User Growth Trajectory")
            st.plotly_chart(px.line(df.user_national_growth(), x='Year', y='Total_Users', markers=True), use_container_width=True)
            
        st.subheader("3. Brand Preference by State")
        st.plotly_chart(px.bar(df.user_state_brand_matrix(year), x="State", y="Users", color="Brand"), use_container_width=True)
        
        st.subheader("4. Engagement Bubble Chart")
        st.plotly_chart(px.scatter(df.user_app_opens_dist(year, quarter), x="Registered_Users", y="App_Opens", size="Registered_Users", color="District", hover_name="District"), use_container_width=True)

        st.subheader("5. Top 5 Mobile Brands")
        st.plotly_chart(px.bar(df.user_brand_leaderboard(year), x='Brand', y='Users', text='Users'), use_container_width=True)

    # --- SCENARIO 3: INSURANCE PENETRATION ---
    elif scenario == "3. Insurance Penetration":
        st.subheader("1. Insurance Market Growth")
        st.plotly_chart(px.line(df.ins_trend_national(), x="Year", y="Policies_Sold", markers=True), use_container_width=True)
        
        c1, c2 = st.columns(2)
        with c1:
            st.subheader("2. State Penetration Map")
            st.plotly_chart(plot_india_map(df.ins_state_penetration(year), 'Policies_Sold', 'State', "Where is insurance popular?"), use_container_width=True)
        with c2:
            st.subheader("3. Avg Premium Size")
            st.plotly_chart(px.bar(df.ins_avg_premium(year).head(10), x='State', y='Avg_Premium', color='Avg_Premium'), use_container_width=True)
            
        st.subheader("4. District Hotspots")
        st.plotly_chart(px.bar(df.ins_district_hotspots(year), x='Policies_Sold', y='District', orientation='h'), use_container_width=True)

        st.subheader("5. YoY Growth Rate")
        st.plotly_chart(px.bar(df.ins_growth_vs_prev_year(year), x='Year', y='Total_Amount', text='Total_Amount'), use_container_width=True)
    ...
    # --- SCENARIO 4: MARKET EXPANSION ---
        ...
        with c1:
            ...
            st.plotly_chart(px.bar(df.market_growth_states(year).head(10), x='State', y='Total_Amount', title="Volume Leaders"), use_container_width=True)
        with c2:
            st.subheader("2. Laggard States")
            st.plotly_chart(px.bar(df.market_laggard_states(year), x='State', y='Total_Amount', title="Needs Attention"), use_container_width=True)
            
        st.subheader("3. YoY Growth Rate Leaders (%)")
        st.plotly_chart(px.bar(df.market_state_growth_yoy(year), x='State', y='Growth_Rate', color='Growth_Rate'), use_container_width=True)
        
        st.subheader("4. High Volume / Low Value Markets")
        st.plotly_chart(px.scatter(df.market_high_vol_low_val(year), x='Volume', y='Avg_Ticket_Size', text='State', size='Volume'), use_container_width=True)
        ...
        st.plotly_chart(px.treemap(df.market_district_potential(target_state, year), path=['District'], values='Total_Amount'), use_container_width=True)
    ...
    # --- SCENARIO 5: USER ENGAGEMENT DEEP DIVE ---
        ...
        st.plotly_chart(px.scatter(df.engage_opens_vs_reg(year, quarter), x="Reg_Users", y="App_Opens", size="Reg_Users", color="State"), use_container_width=True)
        
        c1, c2 = st.columns(2)
        with c1:
            st.subheader("2. Total App Opens Ranking")
            st.plotly_chart(px.bar(df.engage_state_ranking(year).head(10), x='State', y='Total_Opens'), use_container_width=True)
        with c2:
            st.subheader("3. Efficiency (Opens Per User)")
            st.plotly_chart(px.bar(df.engage_state_efficiency(year).head(10), x='State', y='Opens_Per_User', color='Opens_Per_User'), use_container_width=True)
        ...
        st.plotly_chart(px.bar(df.engage_district_rate(year, target_state_en).head(10), x='District', y='Engagement_Rate'), use_container_width=True)

        st.subheader(f"5. Quarterly Consistency in {target_state_en}")
        st.plotly_chart(px.line(df.engage_consistency(target_state_en, year), x='Quarter', y='Total_Opens', markers=True), use_container_width=True)
    ...
    # --- SCENARIO 6: INSURANCE ENGAGEMENT ---
        ...
        st.plotly_chart(px.scatter(df.ins_opportunity_gap(year), x="Total_Users", y="Policies_Sold", text="State", size="Total_Users"), use_container_width=True)
        
        c1, c2 = st.columns(2)
        with c1:
            st.subheader("2. Seasonal Trends")
            st.plotly_chart(px.bar(df.ins_seasonal_trend(year), x='Quarter', y='Policies_Sold'), use_container_width=True)
        with c2:
            st.subheader("3. Fastest Growing States (YoY)")
            st.plotly_chart(px.bar(df.ins_state_growth(year), x='State', y='Growth_Pct', color='Growth_Pct'), use_container_width=True)
        ...
        st.plotly_chart(px.treemap(df.ins_district_penetration(target_state_ins, year), path=['District'], values='Count'), use_container_width=True)

        st.subheader("5. Top 10 Insurance Districts (National)")
        st.plotly_chart(px.bar(df.ins_top_district_penetration(year), x='District', y='Total_Policies'), use_container_width=True)
    ...
    # --- SCENARIO 7: TRANSACTION LEADERBOARDS ---
        ...
        with c1:
            ...
            st.plotly_chart(px.bar(df.top_trans_states(year, quarter), x='State', y='Total_Count'), use_container_width=True)
        with c2:
            st.subheader("2. Top 10 Districts")
            st.plotly_chart(px.bar(df.top_trans_districts(year, quarter), x='District', y='Transaction_Count', color='Transaction_Count'), use_container_width=True)
            
        st.subheader("3. Top 10 Pincodes (Hyper-local)")
        st.plotly_chart(px.bar(df.top_trans_pincodes(year, quarter), x='Pincode', y='Transaction_Count'), use_container_width=True)

        c3, c4 = st.columns(2)
        with c3:
            st.subheader("4. Bottom 10 Districts")
            st.plotly_chart(px.bar(df.bottom_trans_districts(year, quarter), x='District', y='Transaction_Count'), use_container_width=True)
        with c4:
            st.subheader("5. State Pincode Coverage")
            st.plotly_chart(px.bar(df.trans_pincode_reach(year).head(10), x='State', y='Pincodes_Covered'), use_container_width=True)
    ...
    # --- SCENARIO 8: USER LEADERBOARDS ---
        ...
        with c1:
            st.subheader("1. Top 10 States")
            st.plotly_chart(px.bar(df.top_user_states(year, quarter), x='State', y='Total_Users'), use_container_width=True)
        with c2:
            st.subheader("2. Top 10 Districts")
            st.plotly_chart(px.bar(df.top_user_districts(year, quarter), x='District', y='Registered_Users'), use_container_width=True)
            
        st.subheader("3. Top 10 Pincodes (Density)")
        st.dataframe(df.top_user_density_pincode(year, quarter), use_container_width=True)

        c3, c4 = st.columns(2)
        with c3:
            st.subheader("4. Bottom 10 Districts")
            st.plotly_chart(px.bar(df.bottom_user_districts(year, quarter), x='District', y='Registered_Users'), use_container_width=True)
        with c4:
            st.subheader("5. Pincode Share")
            # Using data from top pincodes to show share
            st.plotly_chart(px.pie(df.top_user_density_pincode(year, quarter), names='Pincode', values='Registered_Users'), use_container_width=True)
    ...
    # --- SCENARIO 9: INSURANCE LEADERBOARDS ---
        ...
        with c1:
            st.subheader("1. Top 10 States")
            st.plotly_chart(px.bar(df.top_ins_states(year, quarter), x='State', y='Policies_Sold'), use_container_width=True)
        with c2:
            st.subheader("2. Top 10 Districts")
            st.plotly_chart(px.bar(df.top_ins_districts(year, quarter), x='District', y='Insurance_Count', color='Insurance_Count'), use_container_width=True)
            
        st.subheader("3. Top 10 Pincodes (High Value)")
        st.plotly_chart(px.bar(df.top_ins_pincode_value(year, quarter), x='Pincode', y='Insurance_Amount'), use_container_width=True)

        c3, c4 = st.columns(2)
        with c3:
            st.subheader("4. Bottom 10 Districts")
            st.plotly_chart(px.bar(df.bottom_ins_districts(year, quarter), x='District', y='Insurance_Count'), use_container_width=True)
        with c4:
            st.subheader("5. Top Pincodes (High Count)")
            st.dataframe(df.top_ins_pincodes(year, quarter), use_container_width=True)
